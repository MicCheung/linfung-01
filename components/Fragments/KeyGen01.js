...
export const KeyGen01a = ({section, downloads}) => {
    ...
    if (!nosys) {
      sendData(allWords[0].form,userName,userMail,serial,"trialserials");
    }
                ...
                <div key={'f005'+section.key+i}  className="grid grid-cols-5 md:grid-cols-7 border-1 border-slate-400 rounded-md">
                    ...
                    <div key={'f008'+section.key+i}   className = {` col-span-3 md:col-span-5 border border-slate-400 px-5 rounded-md py-2   ${ i===noItems ? 'block' : 'hidden'}  ${ nosys ? 'hidden' : 'block'}  `}>
                        ...
                        <Link 
                            ...
                            className="">
                            <h3 key={'f010'+section.key+i}  className="w-36 inline-flex outline-4 cursor-pointer justify-center py-3 mr-5 my-1 border border-transparent shadow text-base text-xl md:text-xl rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"> 
                                {`${source[1]}`}   
                            </h3>
                        </Link>
                         ))}    
...
dynamic(() => Promise.resolve(KeyGen01a),{ssr:false});
...
function getOS() {
 ...
 function getNumber() {
    let num = '';
    for (var i = 0; i < 30; i++) {
        num = num + Math.floor(Math.random()*10).toString()  
    }
    return num;
 }
